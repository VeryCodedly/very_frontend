...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/store/products/${slug}/`)
      .then(res => res.json())
      .then(data => {
        setProduct(data);
        const active = data.variant_mapping.filter((v: Variant) => v.active);
        setActiveVariants(active);

        const firstVariant = active[0];
        setSelectedImage(firstVariant?.preview_image || data.preview_image);
        setSelectedThumbnail(firstVariant?.thumbnail_url || data.thumbnail_url);

        const sizes = [...new Set<string>(active.map((v: Variant) => v.size))];
        if (sizes.length) setSelectedSize(sizes[0]);

        // Build colors from the local `active` array – NOT from state
        const uniqueColors = [
          ...new Map<string, Variant>(
            active.map((v: Variant) => [v.color, v])
          ).entries(),
        ].map(([color, variant]) => ({
          name: color,
          value: colorValueMap[color as keyof typeof colorValueMap] || color,
          previewImage: variant.preview_image,
        }));
        
        if (uniqueColors.length) setSelectedColor(uniqueColors[0].name); // ← restore this line
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/store/products/?category=${product.category}`)
      .then(res => res.json())
  ...
  const uniqueColors = [...new Map(activeVariants.map(v => [v.color, v])).entries()]
    .map(([color, variant]) => ({ name: color, value: colorValueMap[color] || color, previewImage: variant.preview_image }));
  ...
  {/* Floating Cart Button */}
      ...
      <div className="max-w-xl md:max-w-5xl mx-auto px-6 py-6 pb-16">
          ...
          <div className="space-y-5.5">
            ...
            <div className="pb-5 pt-2">
              <button
                ...
                active:bg-white active:text-black shadow-[0_4px_0_0_#39ff14] hover:shadow-[0_2px_0_0_#39ff14] text-base sm:text-l
        ...
        {relatedProducts.length > 0 && (
          <section className="my-8 group/bar">
            <div className="flex items-center gap-3 mb-6">
